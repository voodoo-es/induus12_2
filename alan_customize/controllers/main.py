            ...
            for i in enable:
                if i.startswith('theme_alan.header_layout_'):
                    if not i.endswith('_css'):
                        ...
                        views = request.env['ir.ui.view'].search(
                            [('key', 'ilike', 'theme_alan.header_layout_')])
                        for v in views:
                ...
                if i.startswith('theme_alan.footer_layout_'):
                    if not i.endswith('_css'):
                        templ = i + '_css'
                        views = request.env['ir.ui.view'].search(
                            [('key', 'ilike', 'theme_alan.footer_layout_')])
                        for v in views:
                            if v.key.endswith('_css'):
                                set_active([v.key], False)
                        set_active([templ], True)
...
class WebsiteSale(WebsiteSale):
    ...
    def _get_search_order(self, post):
        return 'is_published desc,%s , name, id desc' % post.get('order', 'website_sequence')
    ...
    def load_next_products(self, category='', loaded_products=0, search='', ppg=0, **post):
        if ppg:
            ...
            products = Product.search(domain, limit=int(ppg), offset=int(loaded_products),
