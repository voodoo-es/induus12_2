    ...
    $(document).on('click', '#load_more_products', function(){
        ...
        $.get("/shop/load_next_products", param_dict).then(function(data){
            if(data.trim()){
                ...
                var productos_cargados = $("div#products_grid div.as-product-list > .row:first > div > form[action='/shop/cart/update']").length;
                $btnEle.siblings("input[name='loaded_products_count']").val(productos_cargados);
                $("#productos_cargados").html(productos_cargados);
