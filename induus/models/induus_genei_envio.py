...
class GeneiEnvio(models.Model):
    ...
    cod_promo = fields.Char('Códgio promocional')
    ...
    def generar(self):
        for r in self:
            ...
            for k, v in list(reversed(list(sorted(horarios['inicial'].items()))))[0:3]:
                hora_recogida_desde = v

            hora_recogida_hasta = None
            for k, v in list(reversed(list(sorted(horarios['final'].items()))))[0:3]:
                hora_recogida_hasta = v
            
            _logger.warning("Horarios")
            _logger.warning(hora_recogida_desde)
            _logger.warning(hora_recogida_hasta)
            observaciones_llegada = ""
            if r.dni_llegada:
                observaciones_llegada = "CIF:  %s" % r.dni_llegada

            if r.observaciones_llegada:
                observaciones_llegada = "\n%s" % r.observaciones_llegada
