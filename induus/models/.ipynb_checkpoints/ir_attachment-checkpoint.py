...
class Attachment(models.Model):
    ...
    date_invoice = fields.Date(related="invoice_id.date_invoice", string="Fecha factura")
    invoice_reference = fields.Char(related="invoice_id.reference", string="Referencia")
    ...
    def _compute_invoice(self):
        for r in self:
            if r.res_model == 'account.invoice' and r.res_id:
                invoice = self.env['account.invoice'].search([('id', '=', r.res_id)], limit=1)
        ...
        for id in invoice_ids:
            ...
            if invoice:
                ...
                if invoice.date_invoice:
                    ...
                    ano = str(invoice.date_invoice.year)
                    ...
                    mes = date_utils.nombre_mes(invoice.date_invoice)
                    tag = DocumentTag.search([('name', '=', mes)], limit=1)
                    if tag:
                        tag_ids.append((4, tag.id))

                    trimestre = None
                    if invoice.date_invoice.month >= 1 and invoice.date_invoice.month <= 3:
                        trimestre = self.env.ref("induus.documents_tag_1trimestre")
                    elif invoice.date_invoice.month >= 4 and invoice.date_invoice.month <= 6:
                        trimestre = self.env.ref("induus.documents_tag_2trimestre")
                    elif invoice.date_invoice.month >= 7 and invoice.date_invoice.month <= 9:
                        trimestre = self.env.ref("induus.documents_tag_3trimestre")
                    elif invoice.date_invoice.month >= 10 and invoice.date_invoice.month <= 12:
