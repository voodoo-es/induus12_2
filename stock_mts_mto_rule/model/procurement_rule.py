...
class ProcurementRule(models.Model):
    ...
    def _run_split_procurement(self, product_id, product_qty, product_uom, location_id, name, origin, values):
        needed_qty_original = self.get_mto_qty_to_order(product_id, product_qty, product_uom, values)
        needed_qty = needed_qty_original

        if needed_qty_original == 0.0:
            getattr(self.mts_rule_id, '_run_%s' % self.mts_rule_id.action)(
                product_id, product_qty, product_uom, location_id, name,
                origin, values)
        elif needed_qty_original == product_qty:
            getattr(self.mto_rule_id, '_run_%s' % self.mto_rule_id.action)(
                product_id, needed_qty, product_uom, location_id, name,
                origin, values)
        else:
            mts_qty = product_qty - needed_qty_original
            getattr(self.mts_rule_id, '_run_%s' % self.mts_rule_id.action)(
