...
class KsDashboardNinjaItems(models.Model):
    ...
    def ks_get_record_count(self):
        for rec in self:
            if rec.ks_record_count_type == 'count':
                ...
                rec.ks_record_count = ks_records_grouped_data.get(rec.ks_record_field.name,0) if ks_records_grouped_data.get('__count',False) and (ks_records_grouped_data.get(rec.ks_record_field.name)) else 0
            elif rec.ks_record_count_type == 'average' and rec.ks_record_field:
                ks_records_grouped_data = rec.ks_fetch_model_data(rec.ks_model_name, rec.ks_domain, 'read_group', rec)[0]
                rec.ks_record_count = ks_records_grouped_data.get(rec.ks_record_field.name,0)/ks_records_grouped_data.get('__count',1) if ks_records_grouped_data.get('__count',False) and (ks_records_grouped_data.get(rec.ks_record_field.name)) else 0
            else:
                ...
                if rec.ks_record_count_type == 'count':
                    ...
                    ks_record_count = ks_records_grouped_data.get(rec.ks_record_field.name,
                                                                      0) if ks_records_grouped_data.get('__count',
                                                                                                        False) and (ks_records_grouped_data.get(rec.ks_record_field.name)) else 0
                elif rec.ks_record_count_type == 'average' and rec.ks_record_field:
                    ks_records_grouped_data = rec.ks_fetch_model_data(rec.ks_model_name, rec.ks_domain, 'read_group', rec)[0]
                    ks_record_count = ks_records_grouped_data.get(rec.ks_record_field.name,
                                                                      0) / ks_records_grouped_data.get('__count',
                                                                                                       1) if ks_records_grouped_data.get(
                        '__count', False) and (ks_records_grouped_data.get(rec.ks_record_field.name)) else 0
                else:
                    ks_record_count = 0
    ...
    def ks_get_model_2_data(self, ks_model_name_2, ks_record_count_type_2, ks_record_field_2, ks_domain, rec):
        if rec.ks_record_count_type_2 == 'count':
            ...
            ks_record_count = ks_records_grouped_data.get(ks_record_field_2.name,0) if ks_records_grouped_data.get('__count',False) and (ks_records_grouped_data.get(ks_record_field_2.name)) else 0
        elif ks_record_count_type_2 == 'average' and ks_record_field_2:
            ks_records_grouped_data = rec.ks_fetch_model_data_2(ks_model_name_2, ks_domain, 'read_group', rec)[0]
            ks_record_count = ks_records_grouped_data.get(ks_record_field_2.name,0)/ks_records_grouped_data.get('__count',1) if ks_records_grouped_data.get('__count',False) and (ks_records_grouped_data.get(ks_record_field_2.name)) else 0
        else:
